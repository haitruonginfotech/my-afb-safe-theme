...
class DataCountdown extends HTMLElement {
  ...
  onCountDown() {
    ...
    const numberClass = this.getAttribute('data-number-class') || '';
    const labelClass  = this.getAttribute('data-label-class')  || '';

    const countDownItem = (value, label, itemClass) => {
      const numCls  = ['countdown__number', numberClass].filter(Boolean).join(' ');
      const textCls = ['countdown__text',   labelClass ].filter(Boolean).join(' ');
      return `<div class="countdown__item ${itemClass}"><span class="${numCls}">${value}</span> <span class="${textCls}">${label}</span></div>`;
    };
    const date = new Date(
    ...
    const countDownInterval = setInterval(function () {
      ...
      CountDownElement.innerHTML =
        countDownItem(daysValue, window.countdown.days, "Days") +
        countDownItem(hoursValue, window.countdown.Hours, "Hrs") +
        countDownItem(minutesValue, window.countdown.minutes, "Min") +
        countDownItem(secondsValue, window.countdown.second, "Sec");
